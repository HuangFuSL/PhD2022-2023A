...
class TextModel(C_Module):
    ...
    def __init__(self, hidden_dim: int, num_layers: int):
        super().__init__()
        self.bert = C_BertModel.from_pretrained(
...
referenceId_encode = functools.partial(field_encode, field_name='referenceId')
referenceName_encode = functools.partial(
    field_encode, field_name='referenceName')
productColor_encode = functools.partial(
    field_encode, field_name='productColor')

# ---------------------- BERT WORD EMBEDDING -----------------------

STATIC_BERT = C_BertModel.from_pretrained('bert-base-chinese')  # type: ignore
...
def preprocess_features(data: T_Record, lda: C_LDAModel, dict_: C_Dictionary, topn: int):
    categories_field = LDA_predict(lda, dict_, data['content'])
    referenceId_field = referenceId_encode(data)
    referenceName_field = referenceName_encode(data)
    productColor_field = productColor_encode(data)
...
def filter_words(word: str) -> bool:
    return bool(word) and (word not in set('!@#$%^&*()_+-=,./<>?;:\'"[]{}\\|`~！@#￥%……&*（）——+-=【】、；：”“’。，、？～'))
...
def preprocess_comments(c: T.List[T_Record]) -> T.Dict[str, T.List[T.Any]]:
    ...
    comments['content'] = [
        list(filter(filter_words, c))
        for c in comments['content']
    ]
    comments['tokens'] = preprocess_tokens(comments['content'])
    return comments
...
def build_dataset():
    ...
    rep = functools.partial(replicate, criteria=lambda _: _['score'] < 4, rep=10)
    _train, _test = F_Split(list(
        filter_fields(itertools.chain(*_), LV2_FIELDS),
    ), test_size=0.2)
    train, test = map(preprocess_comments, [list(rep(_train)), _test])  # type: ignore
    dictionary, lda = LDA_fit(train['content'])
...
if __name__ == '__main__':
    ...
    model = UnifiedModel(
        ...
        enable_fm=False, enable_dnn=True, enable_text=True
    ).to(DEV)
    for _ in range(5):
